...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("/home/jeremylongshore/bobs-brain/logs/migration.log"), logging.StreamHandler()],
)
...
class SafeMigrationManager:
    ...
    def connect_firestore(self) -> bool:
        ...
        try:
            self.firestore_db = firestore.Client(project=self.project_id, database=self.database_id)
    ...
    def migrate_collection(self, collection_name: str) -> Dict:
        ...
        stats = {"collection": collection_name, "total": 0, "migrated": 0, "duplicates": 0, "errors": 0}
        ...
        try:
            ...
            logger.info(
                f"   Total: {stats['total']}, Migrated: {stats['migrated']}, Duplicates: {stats['duplicates']}, Errors: {stats['errors']}"
            )
    ...
    def verify_migration(self) -> bool:
        ...
        try:
            ...
            firestore_docs = (
                self.firestore_db.collection("shared_knowledge").where("source", "==", "chromadb_migration").get()
            )
            ...
            if firestore_count == self.migration_stats["migrated_new"]:
                ...
                logger.warning(
                    f"⚠️ Count mismatch: Expected {self.migration_stats['migrated_new']}, Found {firestore_count}"
                )
                return False
