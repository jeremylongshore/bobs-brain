...
def send_query_to_bob(query):
    ...
    event = {
        "type": "event_callback",
        "event": {"type": "message", "text": query, "user": "U_ADMIN_CHECK", "channel": "C_SYSTEM", "ts": "1234567890"},
    }

    try:
        response = requests.post(
            f"{BASE_URL}/slack/events", json=event, headers={"Content-Type": "application/json"}, timeout=15
        )
