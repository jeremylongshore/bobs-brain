...
class BobFinal:
    ...
    async def search_knowledge_graph(self, query):
        ...
        try:
            ...
            results = await self.graphiti.search(query=query, num_results=3)
            ...
            if any(word in query_text.lower() for word in ["shop", "mechanic", "garage"]):
                ...
                for row in results:
                    if row.avg_overcharge > 100:
                        predictions.append(f"⚠️ {row.shop_name}: Typically overcharges by ${row.avg_overcharge:.2f}")
            ...
            for doc in docs:
                ...
                if any(word in str(data).lower() for word in query_lower.split()):
                    if "repair_type" in data and "quoted_price" in data:
                        relevant_data.append(f"{data['repair_type']}: ${data['quoted_price']}")
...
Response:"""
            ...
            if self.model:
                response = self.model.generate_content(
                    prompt, generation_config=GenerationConfig(temperature=0.7, max_output_tokens=1024)
                )
            ...
            loop.run_until_complete(self.store_in_knowledge_graph(text, final_response, user))
