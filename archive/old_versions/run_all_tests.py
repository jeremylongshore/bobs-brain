...
class BobTestRunner:
    ...
    def generate_report(self):
        ...
        report.append(
            f"- {'✅' if has_firestore else '⚠️'} **Firestore**: {'Connected' if has_firestore else 'Not verified'}"
        )
        ...
        for test in self.results["tests_run"]:
            status_icon = {"passed": "✅", "failed": "❌", "timeout": "⏱️", "error": "💥"}.get(test["status"], "❓")
            ...
            if test["output"]:
                ...
                summary_lines = [
                    line for line in output_lines if "✅" in line or "❌" in line or "complete" in line.lower()
                ]
                if summary_lines:
                    report.append(f"\n**Key Results:**")
