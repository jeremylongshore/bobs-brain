...
def health_check():
    ...
    return jsonify({"status": "healthy", "service": "circle-of-life-scraper", "timestamp": datetime.now().isoformat()})
...
def trigger_scraping():
    ...
    try:
        ...
        if scrape_type == "overnight":
            ...
            results = loop.run_until_complete(scraper.skidsteer_scraper.scrape_skidsteer_forums())

        loop.close()

        return jsonify({"status": "success", "scrape_type": scrape_type, "results": results}), 200
...
def get_todays_insights():
    ...
    try:
        ...
        for row in client.query(query).result():
            top_problems.append({"type": row.problem_type, "count": row.count})
...
def search_s740_knowledge():
    ...
    try:
        ...
        return jsonify({"query": query_text, "results": results, "count": len(results)})
...
def setup_scheduler():
    ...
    try:
        ...
        return jsonify({"status": "success", "message": "Cloud Scheduler configured for 2 AM daily runs"})
    except Exception as e:
