...
class BobDualMemory:
        ...
        try:
            ...
            for doc in recent_docs:
                ...
                if not data.get("processed_by_bob", False):
                    new_data.append({"id": doc.id, "data": data, "type": "customer_submission"})
    ...
    async def search_all_memory(self, query):
        ...
        results = {"graphiti": [], "firestore": [], "customer_data": []}
            ...
            for doc in docs:
                ...
                if query.lower() in content.lower():
                    results["firestore"].append(
                        {"id": doc.id, "content": content[:200], "metadata": data.get("metadata", {})}
                    )
...
def sync_data():
    ...
    try:
        ...
        return jsonify({"status": "success", "message": "Firestore data synced to Graphiti"})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def customer_webhook():
    ...
    try:
        ...
        return jsonify({"status": "success", "message": "Customer data received and processed", "id": doc_ref[1].id})
