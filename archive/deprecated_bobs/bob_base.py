...
class BobBase(BobFirestore):
    ...
    def _build_enhanced_context(self, message: str, user_profile: Dict, history: List[Dict]) -> Dict:
        ...
        for item in history[:3]:
            context["relevant_history"].append(
                {"content": item.get("content", "")[:200], "relevance": item.get("relevance", 0)}
            )
    ...
    def get_capabilities(self) -> Dict:
        ...
        return {**self.capabilities, "memory_stats": self.enhanced_memory.get_stats(), "health": self.health_check()}
