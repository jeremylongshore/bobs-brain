...
class BobFirestore:
    ...
    def setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[logging.FileHandler(f"{log_dir}/bob_firestore.log"), logging.StreamHandler(sys.stdout)],
        )
    ...
    def setup_databases(self):
        ...
        if FIRESTORE_AVAILABLE:
            try:
                ...
                self.firestore_db = firestore.Client(project=project_id, database=database_id)
                ...
                try:
                    docs = (
                        self.firestore_db.collection("shared_knowledge")
                        .where("ai_agent", "==", "bob")
                        .limit(1000)
                        .get()
                    )
        ...
        if not self.firestore_db and CHROMADB_AVAILABLE:
            try:
                chroma_path = (
                    "chroma_data" if os.path.exists("/app") else "/home/jeremylongshore/bobs-brain/chroma_data"
                )
    ...
    def is_jeremy(self, user_id: str) -> bool:
        ...
        return user_id == self.business_context.get("owner_user_id") or self.conversation_memory.get(
            "user_context", {}
        ).get(user_id, {}).get("is_owner", False)
    ...
    def query_knowledge_chromadb(self, query: str, max_results: int = 5) -> List[Dict]:
        ...
        try:
            results = self.chroma_collection.query(query_texts=[query], n_results=max_results)
            ...
            for doc, meta, dist in zip(
                results["documents"][0], results.get("metadatas", [[]])[0], results["distances"][0]
            ):
                if 1.0 - dist > 0.3:
                    knowledge.append({"content": doc, "metadata": meta, "relevance": 1.0 - dist, "source": "chromadb"})
            return knowledge
    ...
    def update_conversation_memory(self, user_id: str, message: str, response: str = None):
        ...
        if any(g in message.lower() for g in ["hello", "hi", "hey", "good morning"]):
            ...
            self.conversation_memory["greeting_count"][user_id] = self.conversation_memory["greeting_count"][user_id][
                -5:
            ]
        ...
        self.conversation_memory["conversation_history"][user_id].append(
            {"timestamp": now.isoformat(), "user": message, "bob": response}
        )
        self.conversation_memory["conversation_history"][user_id] = self.conversation_memory["conversation_history"][
            user_id
        ][-10:]
...
Provide a helpful, concise response. Be professional but friendly."""
                ...
                response = self.ai_model.generate_content(
                    prompt, generation_config=GenerationConfig(temperature=0.7, max_output_tokens=300)
                )
    ...
    def handle_slack_event(self, client, request):
        ...
        try:
            ...
            if request.type == "events_api":
                ...
                # Send response
                self.slack_client.chat_postMessage(channel=channel, text=response, thread_ts=event.get("thread_ts"))
    ...
    def start(self):
        ...
        try:
            ...
            while self.is_healthy:
                ...
                self.logger.info(
                    f"💓 Health: DB={health['database']}, Conversations={health['active_conversations']}, Messages={health['processed_messages']}"
                )
