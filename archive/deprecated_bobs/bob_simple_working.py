...
"""
                ...
                if "hello" in text.lower() or "hi" in text.lower():
                    ...
                    return (
                        ...
                        + text
                        + "'. I'm having connection issues with my AI model right now, but I'm listening!"
                    )
        except Exception as e:
...
def test():
    ...
    return jsonify({"question": "What is 2+2?", "response": response, "model_loaded": bob.model is not None})


@app.route("/", methods=["GET"])
def index():
    return jsonify({"service": "SimpleBob", "status": "running", "model_loaded": bob.model is not None})
