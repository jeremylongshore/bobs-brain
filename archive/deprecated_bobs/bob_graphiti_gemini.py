    ...
    async def _get_ml_predictions(self, text: str) -> str:
        ...
        try:
            ...
            if any(word in text.lower() for word in ["price", "cost", "expensive", "cheap"]):
                ...
                extract_prompt = (
                    f"Extract the repair type from this question. Return ONLY the repair type or 'unknown': {text}"
                )
    ...
    return jsonify(
        {
            ...
            "models": {"gemini-1.5-flash": "Fast responses", "gemini-1.5-pro": "Deep analysis"},
        }
    )
...
def analyze():
    ...
    try:
        ...
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        analysis = loop.run_until_complete(bob.analyze_with_gemini_pro(data))
...
def test_ml():
    ...
    try:
        ...
        predictions = []
        for row in results:
            predictions.append({"repair": row.repair_type, "avg_price": row.avg_price, "samples": row.sample_size})

        return jsonify({"status": "success", "ml_predictions": predictions, "source": "BigQuery ML"})
