...
class BobGCPLatest:
    ...
    async def _vertex_ai_fallback(self, text: str) -> str:
        ...
        try:
            ...
            endpoint = (
                aiplatform.Endpoint.list(filter='display_name="gemini-endpoint"')[0]
                if aiplatform.Endpoint.list()
                else None
            )

            if endpoint:
                response = endpoint.predict(instances=[{"prompt": text}])
    ...
    async def train_automl_model(self, dataset_name: str, target_column: str) -> Dict:
        ...
        try:
            ...
            return {"success": True, "model_name": model.resource_name, "endpoint": endpoint.resource_name}
    ...
    return jsonify(
        {
            ...
            "ml_models": ["BOOSTED_TREE_REGRESSOR", "AUTOENCODER", "MATRIX_FACTORIZATION", "AutoML"],
        }
    )
    ...
    try:
        ...
        if data.get("type") == "event_callback":
            ...
            if event.get("type") == "message" and not event.get("bot_id"):
                ...
                response = loop.run_until_complete(bob.process_message_with_genai(text, user, channel))
...
def train_automl():
    ...
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    result = loop.run_until_complete(bob.train_automl_model(dataset, target))
