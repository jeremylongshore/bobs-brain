...
class BobTestHarness:
    ...
    def __init__(self):
        ...
        try:
            self.chroma_client = chromadb.PersistentClient(path="/home/jeremylongshore/.bob_brain/chroma")
    ...
    def setup_test_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(
                    f'/home/jeremylongshore/bob-consolidation/logs/test_run_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
                ),
    ...
    def test_chromadb_connection(self) -> Dict[str, Any]:
        ...
        result = {"status": "failed", "details": {}, "duration": 0, "errors": []}
        ...
        try:
            ...
            query_results = self.knowledge_collection.query(
                query_texts=[test_query], n_results=3, include=["documents", "metadatas", "distances"]
            )

            result["details"]["test_query"] = test_query
            result["details"]["query_results_count"] = (
                len(query_results["documents"][0]) if query_results["documents"] else 0
            )
            result["details"]["sample_relevance"] = (
                query_results["distances"][0][0]
    ...
    def test_knowledge_search_functionality(self) -> Dict[str, Any]:
        ...
        start_time = time.time()
        result = {"status": "failed", "details": {"queries": []}, "duration": 0, "errors": []}
        ...
        try:
            for query in test_queries:
                ...
                query_info = {
                    ...
                    "best_relevance": 1.0 - query_results["distances"][0][0]
                    if query_results["distances"] and query_results["distances"][0]
                    else 0,
                ...
                self.logger.info(
                    f"  Query '{query}': {query_info['results_count']} results, {query_info['best_relevance']:.3f} relevance"
                )
            ...
            if successful_queries >= 4:  # At least 4 out of 5 should work
                ...
                self.logger.warning(
                    f"⚠️ {test_name} partial success - {successful_queries}/{total_queries} queries successful"
                )
    ...
    def test_business_communication_responses(self) -> Dict[str, Any]:
        ...
        start_time = time.time()
        result = {"status": "failed", "details": {"responses": []}, "duration": 0, "errors": []}
        ...
        try:
            for scenario in test_scenarios:
                ...
                knowledge_results = self.knowledge_collection.query(query_texts=[user_message], n_results=3)
                ...
                theme_matches = sum(1 for theme in scenario["expected_themes"] if theme.lower() in response_lower)
            ...
            avg_theme_score = sum(r["theme_score"] for r in result["details"]["responses"]) / len(
                result["details"]["responses"]
            )
            avg_professional_score = sum(r["professional_tone"] for r in result["details"]["responses"]) / len(
                result["details"]["responses"]
            )
            ...
            if avg_theme_score >= 0.6 and avg_professional_score >= 0.7:
                result["status"] = "passed"
                self.logger.info(
                    f"✅ {test_name} passed - Theme: {avg_theme_score:.2f}, Professional: {avg_professional_score:.2f}"
                )
            else:
                result["errors"].append(
                    f"Quality scores below threshold - Theme: {avg_theme_score:.2f}, Professional: {avg_professional_score:.2f}"
                )
    ...
    def assess_professional_tone(self, response: str) -> float:
        ...
        unprofessional_indicators = ["awesome", "cool", "hey there", "yo", "sup", "lol"]
...
def main():
    ...
    # Save report to file
    report_file = (
        f'/home/jeremylongshore/bob-consolidation/logs/test_report_{datetime.now().strftime("%Y%m%d_%H%M%S")}.md'
    )
