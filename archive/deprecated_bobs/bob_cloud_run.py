    ...
    def query_knowledge(self, query: str, max_results: int = 3) -> List[Dict]:
        ...
        try:
            ...
            for doc in docs:
                ...
                if relevance > 0:
                    results.append({"content": data.get("content", ""), "relevance": relevance / len(query_words)})
...
Provide a helpful, concise response. Be professional but friendly."""
    ...
    def send_response(self, channel: str, text: str, thread_ts: Optional[str] = None):
        ...
        try:
            self.slack_client.chat_postMessage(channel=channel, text=text, thread_ts=thread_ts)
    ...
    return jsonify(
        {
            ...
            "version": "1.0",
            "status": "running",
            "endpoints": {"/health": "Health check", "/slack/events": "Slack events webhook"},
        }
    )
