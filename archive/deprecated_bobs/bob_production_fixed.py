    ...
    def __init__(self):
        ...
        try:
            ...
            self.model = GenerativeModel(
                ...
                generation_config=GenerationConfig(temperature=0.7, max_output_tokens=2048, top_p=0.95, top_k=40),
            )
        ...
        try:
            ...
            try:
                ...
                for row in results:
                    if row.avg_price:
                        predictions.append(
                            f"Market Analysis: Average ${row.avg_price:.2f} (Range: ${row.min_price:.2f}-${row.max_price:.2f}, {row.sample_size} samples)"
                        )
            ...
            if hasattr(self, "automl_endpoint") and self.automl_endpoint:
                try:
                    ...
                    instance = {"repair_type": "brake_replacement", "vehicle_year": 2020, "vehicle_make": "Toyota"}
...
Think step-by-step about the user's needs, then respond:"""
            ...
            if self.model:
                ...
                response = self.model.generate_content(
                    prompt,
                    generation_config=GenerationConfig(temperature=0.7, max_output_tokens=1024, candidate_count=1),
                )
