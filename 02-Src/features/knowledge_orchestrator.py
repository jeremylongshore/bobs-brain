...
class BobKnowledgeOrchestrator:
    ...
    def _init_llamaindex(self):
        ...
        # 1. Setup Settings (LLM for query understanding)
        # Use Ollama with local Llama 3.1 instead of OpenAI
        try:
            ...
            logger.info(f"✅ Using Ollama LLM: {os.getenv('OLLAMA_MODEL', 'llama3.1:8b')}")
        except ImportError:
            logger.warning("Ollama LLM not available, falling back to OpenAI")
