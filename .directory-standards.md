...
## BOB'S BRAIN ACTUAL DIRECTORY STRUCTURE

**Note:** This project follows Python packaging conventions, NOT the numbered directory system.
...
│   ├── bob_brain_v5.py         # Main Flask service
│   ├── circle_of_life.py       # ML learning pipeline
│   ├── unified_scraper_*.py    # Data collection
│   └── skills/                 # Bob's skill modules
├── tests/                      # Test suites (pytest)
│   ├── unit/                   # Unit tests
│   └── integration/            # Integration tests
├── docs/                       # Project documentation
│   ├── deployment/             # Deployment guides
│   └── _posts/                 # GitHub Pages content
...
│   ├── old_src_files/          # Replaced source files
│   ├── old_versions/           # Migration scripts
│   └── test_files/             # Old test files
├── ai-dev-tasks/               # AI development workflow
│   ├── ai-dev-tasks-template-masters/  # PRD templates
│   └── todos/                  # Task tracking
├── tasks/                      # ADR, PRD files
...
1. **Uses `src/` not `02-Src/`** - Python packaging convention
2. **Uses `tests/` not `03-Tests/`** - Pytest convention
3. **Uses `docs/` not `01-Docs/`** - GitHub Pages convention
4. **Uses `scripts/` not `05-Scripts/`** - Standard practice
5. **Uses `archive/` not `99-Archive/`** - Clearer naming

This structure prioritizes:
- Python ecosystem compatibility
- Tool compatibility (pytest, coverage, mypy)
...
- **1.0.7** (2025-10-05) – Bob's Brain specific: Updated to reflect actual Python packaging structure
