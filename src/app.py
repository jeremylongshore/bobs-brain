...
from src.providers import llm_client, state_db, vector_store, graph_db, cache_client, artifact_store
...
@app.before_request
def auth():
    if request.path in OPEN_PATHS: return
...
@app.after_request
def _count(resp):
    try: REQS.labels(request.path).inc()
...
@app.get("/")
def root(): return jsonify({"name":"bobs-brain","version":"5.0","status":"ok"})
...
def health_backends():
    ok = {}
    try: conn = STATE.connect(); conn.close(); ok["state"]="ok"
    except Exception as e: ok["state"]=f"fail:{e}"
    try:
        if GRAPH:
            with GRAPH.session() as s: s.run("RETURN 1")
        ok["graph"] = "ok" if GRAPH else "disabled"
    except Exception as e: ok["graph"]=f"fail:{e}"
...
def api_query():
    ...
    try: guard_request("/api/query", body)
...
def api_skill():
    body = request.get_json(force=True) or {}
    name = body.get("name"); payload = body.get("payload",{})
    fn = SKILLS.get(name)
    if not fn: return jsonify({"error":"unknown skill"}), 404
    out = fn(payload)
...
if os.getenv("COL_SCHEDULE"):
    ...
    def scheduled_col():
        try: COL.run_once([{"type":"heartbeat","ts": time.time()}])
        except Exception as e: log.warning("CoL scheduled run failed: %s", e)
