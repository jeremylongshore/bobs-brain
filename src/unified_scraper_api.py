...
def scrape_youtube():
    ...
    try:
        ...
        try:
            count = loop.run_until_complete(youtube_scraper.search_and_scrape(query, max_results))

            return jsonify({"status": "success", "query": query, "transcripts_scraped": count}), 200
...
def scrape_tsb():
    ...
    try:
        ...
        try:
            equipment_tsbs = loop.run_until_complete(tsb_scraper.scrape_equipment_tsbs())

            truck_tsbs = loop.run_until_complete(tsb_scraper.scrape_diesel_truck_tsbs())
