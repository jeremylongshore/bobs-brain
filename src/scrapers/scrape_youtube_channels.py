...
class CustomYouTubeScraper:
    ...
    def extract_knowledge(self, transcript: str):
        ...
        knowledge = {"steps": [], "parts": [], "codes": [], "tools": []}
            ...
            for channel_info in channels:
                if channel_info["channel_id"] != "NEED_TO_FIND":
                    results = await self.scrape_channel_videos(
                        channel_info["channel_id"], channel_info["channel"], max_videos=10
                    )
...
async def main():
    ...
    print(
        "   SELECT DISTINCT code FROM `bobs-house-ai.youtube_equipment.channel_knowledge`, UNNEST(error_codes) as code WHERE code LIKE 'P%'"
    )
    print("\n2. Get Pine Hollow's most viewed videos:")
    print(
        "   SELECT video_title, view_count FROM `bobs-house-ai.youtube_equipment.channel_knowledge` WHERE channel_name = 'Pine Hollow Auto Diagnostics' ORDER BY view_count DESC"
    )
