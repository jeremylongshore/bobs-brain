...
class CompleteFormIntegration:
    ...
    def process_form_submission(self, form_data: Dict) -> Dict:
        ...
        try:
            ...
            record = self._prepare_complete_record(submission_id, cleaned_data, ai_response, vehicle_id)
    ...
    def _update_vehicle_history(self, vehicle_id: str, record: Dict):
        ...
        try:
            ...
            job_config = bigquery.QueryJobConfig(
                query_parameters=[bigquery.ScalarQueryParameter("vehicle_id", "STRING", vehicle_id)]
            )
            ...
            job_config = bigquery.QueryJobConfig(
                query_parameters=[
                    ...
                    bigquery.ScalarQueryParameter(
                        "sent_at", "TIMESTAMP", datetime.now().isoformat() if email_result.get("success") else None
                    ),
                    bigquery.ScalarQueryParameter(
                        "status", "STRING", "sent" if email_result.get("success") else "failed"
                    ),
                ]
            )
...
class SendGridEmailService:
    ...
    def send_diagnostic_report(self, recipient, subject, html_body, text_body, pdf_attachment=None):
        ...
        if response.status_code == 202:
            return {"success": True, "provider": "sendgrid", "tracking_id": response.headers.get("X-Message-Id", "")}
        else:
            return {"success": False, "provider": "sendgrid", "error": f"{response.status_code}: {response.text}"}
