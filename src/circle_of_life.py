...
class CircleOfLife:
    ...
    def persist(self, insights: List[Dict]) -> Tuple[int,int]:
        ...
        if self.driver:
            ...
            with self.driver.session() as s:
                for ins in insights:
                    if float(ins.get("confidence",0)) < CONFIDENCE_MIN: continue
                    s.run(
                        "MERGE (i:Insight {id:$id}) "
                        "SET i.pattern=$p, i.action=$a, i.confidence=$c, i.ts=timestamp()",
                        id=content_hash(ins), p=ins.get("pattern"), a=ins.get("action"),
    ...
    def run_once(self, events: List[Dict]) -> Dict:
        ...
        return {"ran": True, "batch": len(batch), "insights": len(insights),
                "persist_graph": g, "persist_bq": bq, "applied": applied}
