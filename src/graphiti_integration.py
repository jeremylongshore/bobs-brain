...
class GraphitiMemory:
    ...
    def add_episode(
        self, content: str, source: str = "scraper", metadata: Dict = None, timestamp: datetime = None
    ) -> str:
        """
        ...
        # Generate episode ID
        episode_id = hashlib.md5(f"{content}_{timestamp or datetime.now()}".encode()).hexdigest()
    ...
    def search(self, query: str, num_results: int = 10, point_in_time: datetime = None) -> List[Dict]:
        """
        ...
        with self.driver.session() as session:
            if point_in_time:
                ...
                result = session.run(
                    search_query, query=query, point_in_time=point_in_time.isoformat(), limit=num_results
                )
            else:
                # Current state query
                search_query = """
    ...
    def _extract_entities_pattern(self, content: str) -> List[Dict]:
        ...
        for brand in brands:
            if brand.lower() in content.lower():
                entities.append({"name": brand, "type": "Equipment", "confidence": 0.9})

        # Error codes
        error_codes = re.findall(r"\b[PC][0-9A-F]{4}\b", content.upper())
        for code in error_codes:
            entities.append({"name": code, "type": "ErrorCode", "confidence": 0.95})

        # Part numbers
        part_numbers = re.findall(r"\b[A-Z]{2,4}[0-9]{2,}-[0-9A-Z]{4,}\b", content.upper())
        for part in part_numbers:
            entities.append({"name": part, "type": "Part", "confidence": 0.85})

        return entities

    def _sync_to_bigquery(
        self, episode_id: str, content: str, source: str, entities: List, relationships: List, metadata: Dict
    ):
        """
...
class CircleOfLifeGraphiti:
        ...
        for item in items:
            try:
                ...
                episode_id = self.memory.add_episode(
                    content=content, source=item.get("source", "unknown"), metadata=metadata, timestamp=datetime.now()
                )
    ...
    def learn_from_conversation(self, user_message: str, bob_response: str):
        ...
        metadata = {"type": "conversation", "user_message": user_message, "bob_response": bob_response}

        episode_id = self.memory.add_episode(content=content, source="bob_conversation", metadata=metadata)
