...
class ComprehensiveScraper:
    ...
    def extract_repair_knowledge(self, text):
        ...
        # Tools
        tool_list = ['multimeter', 'scan tool', 'scanner', 'oscilloscope',
                     'pressure gauge', 'test light', 'vacuum pump']
        knowledge['tools'] = [t for t in tool_list if t in text_lower]
        
        # Problems
        problem_patterns = [
            r"won'?t\s+start", r"no\s+start", r"rough\s+idle",
            r"misfire", r"overheating", r"no\s+power",
            r"hydraulic\s+leak", r"oil\s+leak", r"check\s+engine"
        ]
    ...
    async def scrape_reddit(self):
        ...
        for sub_name in subreddits:
            ...
            # Process posts
            for post in posts:
                knowledge = self.extract_repair_knowledge(
    ...
    def generate_summary(self):
        ...
        print(f"  • Total items: {len(self.results['reddit']) + len(self.results['youtube']) + len(self.results['forums'])}")
